...
E3FrHomo<-ordered(df$E3, levels = c("Mantener Lazos", "Romper lazos"))%>%
...
E3AngryHomo <-df%>%
  dplyr::group_by("Cámaras de eco"=HomoIRec, "Tratamiento"=E3Treat)%>%
  dplyr::summarize(Media = mean(E3Angry, na.rm = T), "Desviacion Estandar" = sd(E3Angry, na.rm = T),
                   "Quantil 25%" = quantile(E3Angry, probs = q[1]), "Quantil 50%" = quantile(E3Angry, probs = q[2]),
                   "Quantil 75%" = quantile(E3Angry, probs = q[3]))
...
E2Homo <-df%>%
  dplyr::group_by("Cámaras de eco"=HomoIRec,"Tratamiento"=E2Treat)%>%
  dplyr::summarise(Media = mean(SC0, na.rm = T),
...
#E4M2 <-glm(E4 ~ E4Treat + HomoIRec, data = df, family = "binomial")
#E4M3 <-glm(E4 ~ E4Treat + HomoIRec + DigitIndex, data = df, family = "binomial")
#E4M4 <-glm(E4 ~ E4Treat + HomoIRec + DigitIndex + AgeRecod, data = df, family = "binomial")
#E4M5 <-glm(E4 ~ E4Treat + HomoIRec + DigitIndex + AgeRecod + EducRec, data = df, family = "binomial")
#E4M6 <-glm(E4 ~ E4Treat + HomoIRec + DigitIndex + AgeRecod + EducRec + GenRecod, data = df, family = "binomial")
#E4M7 <-glm(E4 ~ E4Treat + HomoIRec + DigitIndex + AgeRecod + EducRec + GenRecod + ideologia, data = df, family = "binomial")
...
#E4AngryHomo <-df%>%
#  dplyr::group_by("Cámaras de eco"=HomoIRec, "Tratamiento"=E4Treat)%>%
#  dplyr::summarize(Media = mean(E4Angry, na.rm = T), "Desviacion Estandar" = sd(E4Angry, na.rm = T),
#                   "Quantil 25%" = quantile(E4Angry, probs = q[1]), "Quantil 50%" = quantile(E4Angry, probs = q[2]),
#                   "Quantil 75%" = quantile(E4Angry, probs = q[3]))
...
#E4JoyHomo<-df%>%
#  dplyr::group_by("Cámaras de eco"=HomoIRec,"Tratamientos"=E4Treat)%>%
#  dplyr::summarize(Media = mean(E4Joy, na.rm = T), "Desviacion Estandar" = sd(E4Joy, na.rm = T),
#                   "Quantil 25%" = quantile(E4Joy, probs = q[1]), "Quantil 50%" = quantile(E4Joy, probs = q[2]),
#                   "Quantil 75%" = quantile(E4Joy, probs = q[3]))
...
TablaExperi <-list(E1, E1Homo, E1Digit,E2, E2Homo, E2Digit,E3, E3Friend, E3Arg,E3FrHomo,E3FrDigi, E3ArgHomo, E3ArgDigi,E3ReBa,E3ReUn)

TablaEmo <-list(E3Angry, E3AngryHomo, E3AngryDigit, E3Joy, E3JoyHomo, E3JoyDigit, E3Sad, E3SadHomo, E3SadDigit,
                E3Fear, E3FearHomo, E3FearDigit)
nueva <-list(E3Homo,E3Digi)

#testing block randomization

E1BHomo <-ctable(df$HomoIRec, df$E1Treat, dnn = c('Cámaras de Eco', 'Tratamiento'), style = 'rmarkdown', prop = "r",
                 useNA = "no", headings = FALSE)
E1BDigi <-ctable(df$DigitIndex, df$E1Treat, dnn = c('Ciudadanía Digital','Tratamiento'), style = 'rmarkdown', prop = "r",
                 useNA = "no",headings = FALSE)

E2BHomo <-ctable(df$HomoIRec, df$E2Treat,dnn = c('Cámaras de Eco', 'Tratamiento'), style = 'rmarkdown', prop = "r",useNA = "no")
E2BDigi <-ctable(df$DigitIndex, df$E2Treat, dnn = c('Ciudadanía Digital','Tratamiento'), style = 'rmarkdown', prop = "r",
                 useNA = "no",headings = FALSE)

E3BHomo <-ctable(df$HomoIRec, df$E3Treat,dnn = c('Cámaras de Eco', 'Tratamiento'), style = 'rmarkdown', prop = "r",
                 useNA = "no", headings = FALSE)
E3BDigi <-ctable(df$DigitIndex, df$E3Treat, dnn = c('Ciudadanía Digital','Tratamiento'), style = 'rmarkdown', prop = "r",
                 useNA = "no", headings = FALSE)

#E4BHomo <-ctable(df$HomoIRec, df$E4Treat,dnn = c('Cámaras de Eco', 'Tratamiento'), style = 'rmarkdown', prop = "r",
#                 useNA = "no", headings = FALSE)
#E4BDigi <-ctable(df$DigitIndex, df$E4Treat, dnn = c('Ciudadanía Digital','Tratamiento'), style = 'rmarkdown', prop = "r",
#                 useNA = "no", headings = FALSE)

tablaBlock <-list(E1BHomo,E1BDigi,E2BHomo,E2BDigi,E3BHomo,E3BDigi)
...
saveRDS(nueva, file = "Results/Tables/tablas-nuevas.rds")
